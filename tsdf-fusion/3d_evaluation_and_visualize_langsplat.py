import yaml
from language.autoencoder.model import AutoencoderLangsplat, AutoencoderLight
import os
import torch
import torchvision
import open_clip
...
path = '/home/choyingw/Documents/GaussianGripMapping/datasets/replica_v2/room_2/imap/00/render_config.yaml'
# Your langsplat/ langslam result path to load reconstructed TSDF
load_path = '/home/choyingw/Documents/langsplat_results/room2'
# where to save point cloud/ mesh results
pc_save_path = f'{load_path}/3d_mesh'
# auto-encoder path
ae_ckpt_path = "/home/choyingw/Documents/langsplat_results/room2/ae_ckpt/best_ckpt.pth" 
# color matrix for groundtruth class
color_mat = np.load('/home/choyingw/Documents/GaussianGripMapping/datasets/replica_v2/room_2/imap/00/color_code.npy')
# where is the reconstructed groundtruth point cloud
gt_pcd = o3d.io.read_point_cloud("/home/choyingw/Documents/langsplat_results/room2/GT_semantic_pc.ply")
...
# query prompts    
query = ['wall', 'chair', 'floor', 'plate', 'vase', 'window', 'indoor-plant', 'table', 'blinds']
...
# load TSDF reconstructed mesh (only for using the triangular faces)
...
auto_model = AutoencoderLangsplat(encoder_hidden_dims, decoder_hidden_dims, clip_dim=768).to(device)
auto_model.load_state_dict(checkpoint, strict=True)
auto_model.eval()
...
restored_feat_resize = auto_model.decode(sem_feat).view(num_points, -1)
fetch = clip_model.get_semantic_map_pc(restored_feat_resize).cpu().numpy()
