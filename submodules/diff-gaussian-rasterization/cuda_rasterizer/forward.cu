...
language_renderCUDA(const uint2* __restrict__ ranges,
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < LANGUAGE_CHANNELS; ch++) {
				L[ch] += language_features[collected_id[j] * LANGUAGE_CHANNELS + ch] * alpha * T;
			}
	...
	if (inside)
	{
		...
		for (int ch = 0; ch < LANGUAGE_CHANNELS; ch++) {
			out_language_feature[ch * H * W + pix_id] = L[ch] ;//+ T * bg_color[ch]; 
	...
	int* n_touched) 
{
	...
	//std::cout<< "forwardcu: NUM_LANGUAGE_CHANNELS: "<< NUM_LANGUAGE_CHANNELS << std::endl;
}
