	...
	float* __restrict__ dL_ddepths)
{
	...
	//float collected_language_local[F];
	//float collected_dL_dlanguage_feature_local[F];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			#pragma unroll
			for (int i = 0; i < F; i++) {
				collected_language[i * BLOCK_SIZE + tid] = language_feature[coll_id * F + i];
				//collected_language_local[i] = language_feature[coll_id * F + i];
			}
		...
		for (int j = 0; j < min(BLOCK_SIZE, toDo); j++) {
			...
			for (int ch = 0; ch < F; ch++)
			{
				...
				//const float f = collected_language_local[ch];
				// Update last color (to be used in the next iteration)
				...
				collected_dL_dlanguage_feature[ch * BLOCK_SIZE + tid] = skip ? 0.0f : dchannel_dcolor * dL_dchannel_F;
				//collected_dL_dlanguage_feature_local[ch] = skip ? 0.0f : dchannel_dcolor * dL_dchannel_F;
			}
			...
			if (tid == 0) {
				...
				for (int ch = 0; ch < F; ch++) {
					...
					//atomicAdd(&(dL_dlanguage_feature[global_id * F + ch]), collected_dL_dlanguage_feature_local[ch]);
				}
			}
		}
	}
