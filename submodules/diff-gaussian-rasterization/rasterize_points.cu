							...
							const bool debug)
{
	...
	if (P != 0) {
		...
		int language_M = 0;
		// if (language_sh.size(0) != 0) {
		// 	language_M = language_sh.size(1);
		// }

		num_rendered = CudaRasterizer::LanguageRasterizer::forward(
	...
	//std::cout<< "[rasterize points]: out color shape: " << out_color.sizes() << std::endl;
	//std::cout<< "[rasterize points] out language shape: " << out_language_feature.sizes() << std::endl;
